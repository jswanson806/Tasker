...
describe('retrieves a single user and maps applications to user object', function() {
    test('works', async function() {
        const result = await User.get(duplicateUserData.email);
...
describe('removes a user', function() {
    test('works', async function() {
        ...
        await User.remove('u1@email.com');

        // query the same user again
        try {
            const result = await User.get('u1@email.com');
        } catch(err) {
            expect(err instanceof NotFoundError).toBeTruthy();
        }
    })
    ...
    test('error if no valid user found', async function() {
        try {
            await User.remove('nope@email.com');
        } catch(err) {
            expect(err instanceof NotFoundError).toBeTruthy();
        }
    })
...
describe('applies to a job', function () {
    test('works', async function() {
        ...
        // query an existing user
        const result3 = await User.get(duplicateUserData.email);
